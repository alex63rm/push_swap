...
/*   Updated: 2023/02/22 20:11:41 by alejarod         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_stack	*cur;	// not necessary because this head is a LOCAL COPY

	cur = head;
	while (cur->link != NULL)
	{
		if (cur->number > cur->link->number)	// if at any time a number is higher than the next one, it is not sorted
			return (0);
		cur = cur->link;
	}
	return (1);
...
static void	ft_push_swap(t_stack **head_a, t_stack **head_b, int size)
{
	if (size < 2)
		return ;
	else if (size == 2 && ft_lst_issorted(*head_a) == 0)	// just 2 items and not sorted, swap them
		ft_sa(head_a);
...
int	main(int argc, char **argv)
{
	t_stack	*head_a;
	t_stack	*head_b;
	t_utils	utils;		// created this one to keep 2 variables and save lines in free
	int		word;
